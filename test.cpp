...
void assert_unique(const Set<int> s) {
    ...
    size_t os = c.size();
    ASSERT_TRUE(is_sorted(c.begin(), c.end()));
    size_t ns = unique(c.begin(), c.end()) - c.begin();
    EXPECT_EQ(os, ns);
}
...
TEST(correctness, upper_bound) {
    ...
    EXPECT_EQ(v.begin(), v.upper_bound(-1));
    EXPECT_EQ(v.begin(), v.upper_bound(-500));
    EXPECT_EQ(2, *v.upper_bound(1));
    EXPECT_EQ(998, *v.upper_bound(997));
...
TEST(correctness, reverse_iterator_rbeg_to_rend) {
    ...
    a_it--;
    std::cerr << *a.rbegin();
    for (; a_it != a.rend(); ++a_it) std::cerr << *a_it;
}
...
TEST(correctness, clear_empty_copy) {
    Set<int> c;
    auto c1 = c;
    ASSERT_EQ(true, c1.empty());
    c.clear();
    c1.clear();
    ASSERT_EQ(true, c.empty());
    ASSERT_EQ(true, c1.empty());
}

TEST(correctness, one_sided_begin) {
    Set<int> c;
    c.insert(19);
    //*c.begin() = 1100;
    ASSERT_EQ(*c.begin(), 19);
}
TEST(nothrow, erase) {
    Set<dummy_ex> s;
    s.insert(dummy_ex(1));
